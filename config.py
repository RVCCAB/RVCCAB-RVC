    ...
    if torch.cuda.is_available():
        ...
        if device_capability >= 7:
            ...
            for config_file in ["32k.json", "40k.json", "48k.json"]:
                ...
                print(f"fp16_run için ayarlamalar {config_file} dosyadan alındı.")
        ...
        print(
            "CUDA bulunamadı. Nvidia GPU'ya sahip olduğunuza ve CUDA'yı kurduğunuza emin olun."
        )
    ...
    def device_config(self) -> tuple:
        if torch.cuda.is_available():
            ...
            if (
                ...
                print("GPU bulundu:", self.gpu_name, ", fp32 için zorlandı.")
                self.is_half = False
            else:
                print("GPU bulundu: ", self.gpu_name)
                use_fp32_config()
            self.gpu_mem = int(
            ...
            print("Desteklenen Nvidia GPU bulunmadı. MPS seçiliyor.")
            self.device = "mps"
            self.is_half = False
            use_fp32_config()
        else:
            print("Desteklenen Nvidia GPU bulunamadı. CPU seçiliyor.")
            self.device = "cpu"
            self.is_half = False
            use_fp32_config()
